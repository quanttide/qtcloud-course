...
用户进入课程研发 → 课程树已展示蓝图结构
  ├─ 浏览各层级，检查结构是否合理
  ├─ 发现名称/描述不合适 → 点击编辑 → 修改 → 保存
  ├─ 发现缺少阶段/课时 → 点击「+ 新建」→ 填写名称 → 确认
  ├─ 发现顺序不对 → 拖拽调整到正确位置
  ├─ 确认内容完整后 → 逐个节点发布
  └─ 所有节点发布完成 → 课程可供教学引用
```
...
```
用户进入课程研发 → 课程树为空（或已有其他专业）
  ├─ 点击「+ 新建专业」→ 填写专业名称和描述 → 确认
  ├─ 在新专业下点击「+ 新建课程」→ 填写课程信息 → 确认
  ├─ 在课程下点击「+ 新建阶段」→ 填写阶段名称 → 确认
  │   └─ 可继续新建多个阶段，拖拽调整顺序
  ├─ 在阶段下点击「+ 新建课时」→ 填写课时标题和时长 → 确认
  │   └─ 可继续新建多个课时，拖拽调整顺序
  ├─ 逐个编辑课时，填充场景和步骤内容
  ├─ 使用试听预览验证课时效果
  ├─ 确认无误后发布节点
...
适用于 AI 设计大纲 → 人类填充细节的最常见场景。

```
1. 人类描述课程需求
   ↓
2. AI 通过 CLI 生成课程骨架 JSON（Program→Course→Phase→Lesson）
   ↓
3. 骨架 JSON 导入 Studio
   ↓
4. 人类在 Studio 中：
...
这是实际最常用的流程。AI 负责结构设计（骨架），人类负责内容填充（血肉）。
